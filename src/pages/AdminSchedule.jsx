...
const AdminSchedule = () => {
  ...
  const fetchSchedules = async () => {
    try {
      const res = await axios.get(`${API}/api/admin/schedules`, {
  ...
  const confirmDelete = async () => {
    ...
    if (!id) return;
    try {
      await axios.delete(`${API}/api/admin/schedules/${id}`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingId) {
        await axios.put(`${API}/api/admin/schedules/${editingId}`, payload, {
          headers: token ? { Authorization: `Bearer ${token}` } : {},
        });
      } else {
        await axios.post(`${API}/api/admin/schedules`, payload, {
          headers: token ? { Authorization: `Bearer ${token}` } : {},
        });
      }
      handleCloseModal();
                ...
                {daySchedules.length > 0 ? daySchedules.map(item => (
                  <div key={item.id} className={`schedule-item ${getTypeClass(item.type)}`}>
                    ...
                    <div className="hover-actions">
                      ...
                      <button className="action-btn delete" onClick={() => promptDelete(item.id)}>
                        <FaTrashAlt />
                      </button>
                    </div>
                  </div>
