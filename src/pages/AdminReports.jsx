  ...
  const handleSaveChanges = async () => {
    try {
      const res = await fetch(
        `http://localhost:3000/api/admin/reports/${selectedReport.id}`,
        {
          method: "PUT",
      ...
      if (res.ok) {
        setReports((prev) =>
          prev.map((r) =>
            r.id === selectedReport.id ? { ...r, status: statusToUpdate } : r
          )
        );
        ...
        setModal({
          ...
          type: "error",
          title: "Update Failed",
          message: data.message || "Could not update status.",
        });
      }
    } catch (err) {
      setModal({
             ...
             <div className="detail-section">
                <h3 className="section-title text-blue" style={{marginTop: '24px'}}><FileText size={20} /> Incident Overview</h3>
                 <div className="overview-grid">
                    ...
                    <div className="info-box"><label>PRIORITY LEVEL</label><div className="priority-wrapper"><div className={`priority-badge ${selectedReport.priority.toLowerCase()}`}><TriangleAlert size={14} /> {selectedReport.priority}</div></div></div>
